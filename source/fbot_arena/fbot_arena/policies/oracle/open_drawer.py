...
class OracleOpenDrawerPolicy(PolicyBase):
    def __init__(self, openable_obj, openable_obj_handle_name: str, planners: dict[str, CuroboPlanner]=dict()):
        super().__init__()
    ...
    def get_action(self, env: OpenArmBimanualMimicEnv, observation):
        ...
        if self.state == OraclePickPolicyStates.APPROACH_OBJ:
            if self.state_step_counter == 0:
                ...
                '''cuboid_index = None
                for index, cuboid in enumerate(self.planners['left']._static_world_config.cuboid):
                    print(cuboid.name)
                    if cuboid.name == self.openable_obj.name:
                        cuboid_index = index
                        break
                self.planners['left']._static_world_config.cuboid[cuboid_index].pose[:3] = pickup_obj_pose[..., :3, 3].flatten().detach().cpu().tolist()
                self.planners['left']._static_world_config.cuboid[cuboid_index].pose[3:7] = PoseUtils.quat_from_matrix(pickup_obj_pose[..., :3, :3]).flatten().detach().cpu().tolist()'''
        ...
        if self.state == OraclePickPolicyStates.GRASP_OBJ:
            if self.state_step_counter < 20+30:
                offset_pos = torch.as_tensor([[0.0, 0.0, (0.05)/(20+30)]], device='cuda')
                offset_quat = torch.as_tensor([[1.0, 0.0, 0.0, 0.0]], device='cuda')
                offset_pose = PoseUtils.make_pose(pos=offset_pos, rot=PoseUtils.matrix_from_quat(offset_quat))
                ...
                offset_pos = torch.as_tensor([[0.0, 0.0, -0.001]], device='cuda')
                offset_quat = torch.as_tensor([[1.0, 0.0, 0.0, 0.0]], device='cuda')
                offset_pose = PoseUtils.make_pose(pos=offset_pos, rot=PoseUtils.matrix_from_quat(offset_quat))
                self.left_target_pose = self.left_target_pose @ offset_pose
                action[..., :] = env.target_eef_pose_to_action(
