...
class OracleOpenDoorPolicy(PolicyBase):
    def __init__(self, openable_obj, openable_obj_handle_name: str, pull_side: str, door_radius: float, planners: dict[str, CuroboPlanner]=dict()):
        super().__init__()
        self.openable_obj = openable_obj
        self.openable_obj_handle_name = openable_obj_handle_name
        self.pull_side = pull_side
    ...
    def load_grasps(self):
        grasp_poses = []
        grasp_dir = f"{fbot_arena.__path__[0]}/assets/grasps/{self.openable_obj.name}/{self.openable_obj_handle_name}"
        for grasp_file in os.listdir(grasp_dir):
            with open(f"{grasp_dir}/{grasp_file}", 'r') as f:
                grasp_data = yaml.full_load(f)
            if grasp_data['grasp_result']['joint_states']['joints/drive_joint'] < 27.0:
                ...
                offset_pos = torch.as_tensor([[0.0, 0.0, -0.1]], device='cuda')
                offset_quat = torch.as_tensor([[0.0, 0.0, 0.0, 1.0]], device='cuda')
    ...
    def sort_grasp_poses(self, initial_pose, pickup_object_pose):
        return sorted(
            self.grasp_poses,
            key=lambda p: torch.norm((pickup_object_pose @ p)[0, :3, 3] - initial_pose[0, 0, :3, 3], p=2, dim=-1).item()
        )
    ...
    def get_action(self, env: OpenArmBimanualMimicEnv, observation):
        ...
        if self.state == OraclePickPolicyStates.APPROACH_OBJ:
            if self.state_step_counter == 0:
                ...
                pickup_obj_pose = env.get_object_poses()[self.openable_obj.name]
                world_model_pickup_object_pose = self.planners['left'].get_object_pose(object_name=self.openable_obj.name)
            ...
            if (torch.norm(left_pos[0] - left_target_pos[0], p=2) <= 0.02 and torch.norm(left_quat[0] - left_target_quat[0], p=2) <= 0.01) or True:
        ...
        if self.state == OraclePickPolicyStates.GRASP_OBJ:
            if self.state_step_counter < 20+30:
                offset_pos = torch.as_tensor([[0.0, 0.0, (0.1)/(20+30)]], device='cuda')
                ...
                target_angle = ((self.state_step_counter - (50+30))*torch.pi/1000)
                target_y = math.cos(target_angle)*self.door_radius
                target_z = -math.sin(target_angle)*self.door_radius
                previous_angle = ((self.state_step_counter - 1 - (50+30))*torch.pi/1000)
                previous_y = math.cos(previous_angle)*self.door_radius
                previous_z = -math.sin(previous_angle)*self.door_radius
                dy = target_y - previous_y
                dz = target_z - previous_z
                droll = target_angle - previous_angle
                offset_pos = torch.as_tensor([[0.0, dy, dz]], device='cuda')
                roll = torch.as_tensor([droll], device='cuda')
                pitch = torch.zeros_like(roll)
                yaw = torch.zeros_like(roll)
                #offset_quat = PoseUtils.quat_from_euler_xyz(roll, pitch, yaw)
                offset_quat = torch.as_tensor([[1.0, 0.0, 0.0, 0.0]], device='cuda')
                offset_pose = PoseUtils.make_pose(pos=offset_pos, rot=PoseUtils.matrix_from_quat(offset_quat))
                self.left_target_pose = self.left_target_pose @ offset_pose
                action[..., :] = env.target_eef_pose_to_action(
